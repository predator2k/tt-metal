...
class ModelArgs:
    ...
    def get_attn_qkv_program_config(self, mode: Mode, seq_len: int = 1, prefetcher: Prefetcher = None):
        ...
        if mode == Mode.DECODE:
            ...
            # (see device.cpp grid.y clamp); (8,10) trips grid checks.
    ...
    def dram_matmul_config(self, m: int, k: int, n: int, num_cores=None, fused_activation=None):
        ...
        # `get_optimal_dram_bank_to_reader_assignment` allocates an 8×10
        # core range based on DRAM bank topology, even after `compute_with_
        # storage_grid_size` is clamped to 12×8. The 10th row exceeds the
        # MUX-clamped grid → TT_FATAL in program.cpp CB validation. Route
        # Blackhole multichip through the regular 8×8 matmul instead. Cost
