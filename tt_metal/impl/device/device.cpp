...
CoreCoord Device::compute_with_storage_grid_size() const {
    ...
    // P3a.2 patch: Blackhole P300 under MUX dispatch exposes a 12x9 worker grid
    // (row 9 reserved for ETH dispatch). Some matmul auto-configs request up to
    // 8x10 cores; clamp to 8 rows when fabric data-mover (MUX/DM) is enabled so
    // they never overflow. For single-chip / no-MUX the full 10 rows are natively
    // available.
